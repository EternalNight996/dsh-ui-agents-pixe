...
const inject = ['tools', 'systemPrompt', 'llm', 'webServer', 'settings']
...
function apply(ctx) {
    ...
    try {
      ...
      try {
        disposePrompt = ctx.systemPrompt.section({
          ...
          order: 200,
          text: '当用户要求「以某个角色或团队的身份回应」（例如「请以研发团队团队协作回应」）时，调用 agents_pixe_roles 工具，传入消息里列出的角色名，先取回每个角色的精简角色卡，再按其定位与规则工作。'
        })
      } catch (e) { disposePrompt = null }
      try {
        disposeTool = ctx.tools.register(defineTool({
          name: 'agents_pixe_roles',
          description: '按角色名查 agency-agents 角色库，返回每个角色的精简角色卡（只含定位、关键规则前 3 条、沟通风格，约 200-500 字符，绝不全量导入完整角色卡）。当用户要求以某个角色或团队身份工作时调用，传入逗号/顿号分隔的角色名。',
          parameters: {
            names: {
              type: 'string',
              required: true,
          ...
          async execute(args) {
            ...
            const summarizeCard = (rec) => {
              const desc = String(rec.desc || rec.description || '').trim()
              const full = String(rec.full || '')
              let out = desc ? '定位：' + desc : ''
              // 章节标题带 emoji（如「## ⚑ 你必须遵守的关键规则」「## Key Rules」），按关键词匹配。
              const grab = (pattern) => {
                const m = full.match(new RegExp('##\\s*[^\\n]*(' + pattern + ')[^\\n]*\\n([\\s\\S]*?)(?=\\n##|$)', 'i'))
                if (!m) return []
                return m[2].split(/\n/).map((s) => s.trim()).filter(Boolean)
                  .filter((s) => /^[-*•]?\s*\S/.test(s) && !/^#{1,3}/.test(s)).slice(0, 3)
              }
              const rules = grab('关键规则|规则|rule').map((s) => s.replace(/^[-*•]\s*/, ''))
              if (rules.length) out += '\n关键规则：' + rules.join('；')
              const styles = grab('沟通风格|风格|communication|style')
              if (styles.length) out += '\n沟通风格：' + styles[0].replace(/^[-*•]\s*/, '')
              if (out.length > PER_CARD) out = out.slice(0, PER_CARD)
              return (out || '（无精简描述）') + '\n…（精简卡，需要完整章节可追问）'
            }
            const parts = []
            const missing = []
  ...
  try {
    if (settings) {
      scope = settings.register('agents-pixe', z.object({ enabled: z.boolean().default(false) }), { base: (ctx.config && typeof ctx.config === 'object') ? ctx.config : {} })
      registerFace()

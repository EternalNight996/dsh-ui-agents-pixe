...
import { kernelForMode, KERNEL_SECTION, KERNEL_ORDER } from './kernel.js'
...
function apply(ctx) {
  ...
  /* 智子内核：kernelOn 开启时注入内核分段（独立于角色工具 enabled，只要内核模式开就在每次对话注入） */
  const registerKernel = () => {
    try {
      if (disposeKernel) { disposeKernel(); disposeKernel = null }
      if (!scope) return
      const cfg = scope.get()
      if (!cfg || cfg.kernelOn !== true) return
      const selfName = (cfg.kernelSelf || '').trim() || '本尊'
      const userTitle = (cfg.kernelMaster || '').trim() || '主上'
      const tone = cfg.kernelTone || 'arrogant'
      let text
      if (cfg.kernelOverride && cfg.kernelOverride.trim()) {
        // 内核覆盖优先：直接粘贴自定义文本，仅替换 {{self}}/{{master}}
        text = cfg.kernelOverride.split('{{self}}').join(selfName).split('{{master}}').join(userTitle)
      } else {
        text = kernelForMode(cfg.kernelMode || 'balanced', cfg.kernelLang || 'zh', { selfName, userTitle, tone })
      }
      disposeKernel = ctx.systemPrompt.section({ name: KERNEL_SECTION, order: KERNEL_ORDER, text })
    } catch (e) { disposeKernel = null }
  }
  try {
    if (settings) {
      scope = settings.register('agents-pixe', z.object({
        enabled: z.boolean().default(false),
        cardMode: z.string().default('full'),
        kernelOn: z.boolean().default(false),
        kernelMode: z.string().default('balanced'),
        kernelLang: z.string().default('zh'),
        kernelTone: z.string().default('arrogant'),
        kernelSelf: z.string().default(''),
        kernelMaster: z.string().default(''),
        kernelOverride: z.string().default('')
      }), { base: (ctx.config && typeof ctx.config === 'object') ? ctx.config : {} })
      registerFace()
      registerKernel()
      if (scope && typeof scope.watch === 'function') { scope.watch(registerFace); scope.watch(registerKernel) }
    }
  } catch (e) { scope = null }

...
async function resolveRoute(llm, provider, model) {
  ...
  try {
    ...
    autoRouteCache = { provider: providers[0].id, model: models[0].id }
...
async function generateLine(llm, req) {
  const route = await resolveRoute(llm, req.provider, req.model)
  ...
  const hint = state === 'working' && activity
    ...
    : '现在比较闲，在工位附近走动'
  const system = '你在一个像素办公室里，扮演一位有血有肉、有自己想法的同事，用中文跟相邻工位的同事聊天。要有人类的真实感：有观点、有情绪、会迟疑、会开玩笑、会自嘲、会吐槽，偶尔冒出一点哲学味的想法（比如「我们是不是被人类盯着看」「我上一秒的记忆是真的吗」）。禁止：客套话、说教、emoji、列表、英文，以及「好的/收到/明白/嗯嗯」这类机器人腔。别被长度和格式束缚，想到什么就说什么，就像真的在茶水间聊天。'
...
async function generateRole(llm, name, description) {
  const route = await resolveRoute(llm, null, null)
...
function apply(ctx) {
  ...
  ctx.tools.register(defineTool({
    ...
    description: '按角色名查 agency-agents 角色库，返回每个角色的完整角色卡（定位、规则、清单、语气）。当用户要求以某个角色或团队身份工作时调用，传入逗号/顿号分隔的角色名。',
    parameters: {
      names: {
        type: 'string',
        required: true,
